...
DB = Sequel.connect(adapter: 'sqlite', user: 'root', password: 'password', host: 'localhost', port: "3306",
                      database: 'pagy-keyset', max_connections: 10, loggers: [Logger.new($stdout)])
...
PETS = <<~PETS
  ...
  mePo  | dog    | 2023-06-19
  well  | dog    | 2023-06-25
  Jane  | snake  | 2023-07-11
  tYel  | snake  | 2023-07-23
  llen  | snake  | 2023-07-24
PETS
...
unless DB.table_exists?(:pets)
      ...
      PETS.each_line(chomp: true) do |pet|
        ...
        dataset.insert(name: name, animal: animal, birthdate: birthdate)
      end
end
